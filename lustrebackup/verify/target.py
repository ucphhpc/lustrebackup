...
from lustrebackup.shared.base import force_unicode, human_readable_filesize, \
    print_stderr
from lustrebackup.shared.defaults import backup_verify_dirname, \
    bin_source_verify_init, bin_source_verify_list, date_format, \
    last_verified_name
from lustrebackup.shared.fileio import delete_file, make_symlink, \
    makedirs_rec, path_join, pickle, unpickle
...
from lustrebackup.shared.verify import create_checksum, \
    create_inprogress_verify, get_last_verified_timestamp, \
    remove_inprogress_verify
...
def __init_verify(
    ...
    verify_basepath = path_join(
        configuration, meta_basepath, backup_verify_dirname, convert_utf8=False
    )
    ...
    (command_rc, stdout, stderr) = shellexec(
        configuration, ssh_cmd, args=[command]
    )
    if command_rc == 0:
        verify_result = loads(
            stdout, serializer="json", parse_int=int, parse_float=float
        )
    else:
    ...
    verify_timestamp = verify_result.get("snapshot_timestamp", source_timestamp)
    if source_timestamp > 0 and verify_timestamp != source_timestamp:
    ...
    verify_log_idx = 1
    verify_logpath = path_join(
        configuration, verify_basepath, "%d.log" % verify_timestamp
    )
    while os.path.exists(verify_logpath):
        verify_logpath = path_join(
            configuration,
            verify_basepath,
            "%d.log.%d" % (verify_timestamp, verify_log_idx),
        )
        verify_log_idx += 1
    ...
    if not target_snapshot or not resume:
        return (verify_result, target_snapshot, vlogger)

    # Resume from checkpoint if it exists

    snapshot_target_timestamp = target_snapshot.get("timestamp", 0)
    checkpoint_filepath = path_join(
        configuration,
        meta_basepath,
        backup_verify_dirname,
        "%d-%d.pck" % (verify_timestamp, snapshot_target_timestamp),
        convert_utf8=False,
        logger=vlogger,
    )
    ...
    if not checkpoint and os.path.exists(checkpoint_filepath):
        ...
        for path in checkpoint["fs"].keys():
            ...
            if target and target.get("status", False):
                verify_result["fs"][path]["target"] = target
                verified_files += 1
                verified_bytes += target.get("size", 0)

        # Check of checkpoint timestamp and target timestamp matches

        checkpoint_target_timestamp = checkpoint.get(
            "target_snapshot_timestamp", 0
        )
        if snapshot_target_timestamp != checkpoint_target_timestamp:
            msg = (
                "snapshot_target_timestamp: %d" % snapshot_target_timestamp
...
def __checkpoint(
    ...
    checkpoint_path = path_join(
        configuration, verify_basepath, checkpoint_name, logger=vlogger
    )
    status = pickle(configuration, result, checkpoint_path, logger=vlogger)
    if not status:
        retval = False
        msg = "Failed to create checkpoint: %r" % checkpoint_path
        vlogger.error(msg)
        if verbose:
            print_stderr("ERROR: %s" % msg)
    t2 = time.time()
    ...
    msg = (
        "Checkpoint: size: %s/%s, entries: %d/%d, success: %d, failed: %d"
        % (
...
def list_verify(
    configuration, start_timestamp=0, end_timestamp=0, verbose=False
):
    ...
    if start_timestamp == 0:
        ...
        start_timestamp = get_last_verified_timestamp(
            configuration, logger, verbose=verbose
        )
...
def verify(
    ...
    if not os.path.isdir(verify_basepath):
        ...
        if not status:
            msg = (
                "Failed to create backup verify basepath: %r" % verify_basepath
            )
            logger.error(msg)
            if verbose:
                print_stderr("ERROR: %s" % msg)
    ...
    (result, target_snapshot, vlogger) = __init_verify(
        configuration,
    ...
    verify_timestamp = result.get("snapshot_timestamp", source_timestamp)
    verify_datestr = datetime.datetime.fromtimestamp(verify_timestamp).strftime(
        date_format
    )

    if target_snapshot:
        target_timestamp = target_snapshot.get("timestamp", 0)
        result["target_snapshot_timestamp"] = target_timestamp
    ...
    status = create_inprogress_verify(
        configuration,
        vlogger,
        verify_timestamp,
        target_timestamp=target_timestamp,
    ...
    (mountpoint, _) = mount_snapshot(
        configuration, target_snapshot, postfix="inprogress_verify"
    )
    if not mountpoint:
    ...
    target_datestr = datetime.datetime.fromtimestamp(target_timestamp).strftime(
        date_format
    )
    ...
    for path, values in result["fs"].items():
        ...
        curr_time = time.time()
        if last_checkpoint_time < curr_time - checkpoint_interval:
            (status, last_checkpoint) = __checkpoint(
                configuration,
                vlogger,
                total_t1,
                result,
        ...
        if values["checksum"]:
            (status, checksum) = create_checksum(
                configuration, vlogger, target_path, verbose=verbose
            )
            ...
            if status and checksum == values["checksum"]:
                ...
                retval = False
                msg = "Verify checksum failed for: %r" % target_path
                target_result["error"] = msg
                vlogger.error(msg)
                if verbose:
                    print_stderr("ERROR: %s" % msg)
            ...
            target_result["status"] = True
            msg = "Skipping checksum for entry(%s): %r" % (entry_type, path)
            vlogger.debug(msg)
    ...
    if retval:
        ...
        #       on re-runs
        last_verified_timestamp = get_last_verified_timestamp(
            configuration, vlogger, verbose=verbose
        )
        ...
        if last_verified_timestamp < verify_timestamp:
            rel_verify_filepath = path_join(
                configuration,
                backup_verify_dirname,
                "%d-%d.pck" % (verify_timestamp, target_timestamp),
                logger=vlogger,
            )
            status = make_symlink(
                configuration,
            ...
            if not status:
                retval = False
                msg = (
                    "Failed to create last verified symlink:"
                    + " %s -> %s in %r"
                    % (rel_verify_filepath, last_verified_name, meta_basepath)
                )
                vlogger.error(msg)
                if verbose:
                    print_stderr(msg)
    ...
    status = remove_inprogress_verify(
        configuration, vlogger, verify_timestamp, target_timestamp
    )
    if not status:
        retval = False
    ...
    msg = (
        "Verified source snapshot: %d (%s) using %d source changelog(s):\n"
        % (verify_timestamp, verify_datestr, len(snapshot_timestamps))
    )
    ...
    msg += (
        "Total size: %s/%s, entries: %d/%d, success: %d, failed: %d"
        % (
            human_readable_filesize(stats["bytes"] - stats["bytes_pending"]),
            human_readable_filesize(stats["bytes"]),
            (stats["total"] - stats["total_pending"]),
