...
def __add_snapshot_dict(configuration,
    ...
    if not snapshot_name:
        logger.error("Missing 'snapshot_name' in snapshot: %s" % snapshot)
        return False
    ...
    # NOTE: The source lustre fsname is used as snapshot name
    #       at both source and target
    snapshot_fsname = configuration.lustre_fsname
    if snapshot.get('source', {}).get('fsname', ''):
        snapshot_fsname = snapshot['source']['fsname']
    lustrebackup_pattern = snapshot_name_format \
        % {'fsname': snapshot_fsname,
           'timestamp': "([0-9]+)"}
    # NOTE: To support old legacy '-auto-' snapshot name format
    lustrebackup_pattern = lustrebackup_pattern.replace(
        '-backup-', '-(backup|auto)?-')
    lustrebackup_re = re.compile(lustrebackup_pattern)
    lustrebackup_ent = lustrebackup_re.fullmatch(snapshot_name)
    # NOTE: We use timestamp from snapshot name if snapshot was generated by
    #       lustrebackup, otherwise lustre snapshot create time
    if lustrebackup_ent:
        lustrebackup_snapshot = True
        timestamp = int(lustrebackup_ent.group(2))
    else:
